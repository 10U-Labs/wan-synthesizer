...
That is an obligation on the synthesizer, not a list of what a user is owed. A user only cares about WAN PoPs, the sites and the provider regions that home into them, circuits, and 2-vertex-connected. A transit PoP is none of those, because a circuit is what abstracts it.

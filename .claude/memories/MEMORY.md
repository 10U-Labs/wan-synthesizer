...
- [a-circuit-is-owned-one-segment-at-a-time](a-circuit-is-owned-one-segment-at-a-time.md) — a fiber segment is one carrier's when it has a PoP at both ends and a circuit hands off between carriers at any PoP both have; the one-carrier-per-circuit rule the code holds is the defect GitHub issue #170 tracks

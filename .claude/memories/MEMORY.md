...
- [seeding-races-the-routing-deploy](seeding-races-the-routing-deploy.md) — a new per-tenant store resource can fail the first `seed` run on the new PUT, and `HTTP 403` and `HTTP 404` say which deploy is behind

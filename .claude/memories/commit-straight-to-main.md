...
Use commits as the unit when breaking work down, and think in commit ordering rather than branch-and-merge.

### Why there is no branch and no pull request

It is a single-maintainer repository and the whole history is direct-to-main, so a side branch only adds a merge step. CI on `main` is the verification gate, and with no pull-request buffer it is the only review there is, which is why the tests land in the same commit as the code they cover.

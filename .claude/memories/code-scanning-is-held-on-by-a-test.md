...
reads the setting back with `gh api` and holds the state, the suite and
the two languages, in `documentation.yml`'s `code-scanning` job with
`security-events: read` and `github.token`.

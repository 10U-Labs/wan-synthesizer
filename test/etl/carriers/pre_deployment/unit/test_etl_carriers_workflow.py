...
WORKFLOW = REPO_ROOT / ".github" / "workflows" / "etl_carriers.yml"
LOAD = "load"
...
def test_the_load_runs_the_program() -> None:
    assert [run for run in _steps(LOAD) if "load_carriers" in run and "main(" in run] != []
...
def test_the_loaded_carriers_are_read_back_only_after_the_load() -> None:
    assert _loaded()["jobs"][E2E]["needs"] == [LOAD]


def test_the_e2e_job_runs_the_post_deployment_e2e_tier() -> None:
    assert [run for run in _steps(E2E) if "test/etl/carriers/post_deployment/e2e/" in run] != []
...
@pytest.mark.parametrize("path", [
    ...
    "src/etl/carriers/**",
    "test/etl/carriers/**",
    ".github/workflows/etl_carriers.yml",
])

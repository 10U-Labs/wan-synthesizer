...
WORKFLOW = REPO_ROOT / ".github" / "workflows" / "etl_syntheses.yml"
LOAD = "load"
...
def test_the_load_waits_for_the_regions_load_on_the_same_commit() -> None:
    assert [run for run in _steps(LOAD) if "etl_regions.yml" in run] != []
...
def test_the_load_runs_the_program() -> None:
    assert [run for run in _steps(LOAD) if "load_syntheses" in run and "main(" in run] != []
...
def test_the_created_syntheses_are_read_back_only_after_the_load() -> None:
    assert _loaded()["jobs"][E2E]["needs"] == [LOAD]


def test_the_e2e_job_runs_the_post_deployment_e2e_tier() -> None:
    assert [run for run in _steps(E2E) if "test/etl/syntheses/post_deployment/e2e/" in run] != []
...
@pytest.mark.parametrize("path", [
    ...
    "test/etl/syntheses/**",
    "lib/python/**",
    ".github/workflows/etl_syntheses.yml",
])
def test_a_change_to_the_runs_the_program_or_its_tests_starts_the_workflow(path: str) -> None:
    assert path in _loaded()["on"]["push"]["paths"]


def test_the_workflow_can_be_told_to_create_every_synthesis() -> None:

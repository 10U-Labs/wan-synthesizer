...
What comes back from a search here is what the fiber is then selected against: a separation
becomes a row in the program saying "hold at least this much of the segments that cross
it", so a separation naming the wrong cities or the wrong fiber selects the wrong fiber, and
a separation reported where the fiber already carries what was asked selects fiber nobody
needs.
...
def _whole(*segments: tuple[str, str]) -> dict[tuple[str, str], float]:
    """These fiber segments, each of them held outright."""
...
# The same segment part-held and not held at all. A share of a segment carries that much of
# a way out and no more, and a segment nothing is held of carries none; both are still fiber
# the search would close the separation with, so both are still named.
...
# amount of fiber here can deliver.
...
def test_fiber_already_carrying_what_was_asked_reports_no_separation() -> None:
    ...
    A separation returned here would be selected against, and the fiber it named would be
...
def test_a_segment_held_in_part_carries_only_that_much_of_a_way_out() -> None:
    ...
    A search counting a part-held segment as a whole one would report the requirement met
    and stop the program selecting the rest of it.
    """
...
def test_a_segment_nothing_is_held_of_is_still_the_fiber_to_select() -> None:
    ...
    selecting would close the separation with, and selecting starts from nothing.
    """
...
def test_a_peer_the_fiber_does_not_carry_is_left_out_of_the_count() -> None:
    """A place named as a peer that no fiber reaches ends no way out, so nothing is selected.

    ``a`` still stands on its segment to ``b`` and the search still runs over it; what it
    reports is that there is no fiber to select, rather than a separation naming segments that

...
_READER: dict[str, Any] = {
    "endpoint": "tenants",
    "stored_key": "tenants/f-35/prohibited-circuits.json",
    "stored": [{"source": "Luke, AZ", "target": "Link, TX"}],
    "serve_event": {
        "pathParameters": {"tenant": "f-35"},
        "path": "/x/tenants/f-35/prohibited-circuits",
    },
    "serve_expect": [{"source": "Luke, AZ", "target": "Link, TX"}],
    "unknown_event": {
        "pathParameters": {"tenant": "f-35"},
    ...
    "notbuilt_event": {
        ...
        "path": "/x/tenants/minuteman/prohibited-circuits",
    },
}
...
def test_tenant_delete_leaves_no_delete_marker(monkeypatch: pytest.MonkeyPatch) -> None:
    module = _tenant(monkeypatch)
    fake = fake_s3({"tenants/f-35/prohibited-circuits.json": b"[]", "tenants/f-35/wan.json": b"{}"})
    event = {"httpMethod": "DELETE", "pathParameters": {"tenant": "f-35"}}

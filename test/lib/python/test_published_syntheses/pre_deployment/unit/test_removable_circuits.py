...
from test_published_syntheses import removable_circuits
...
_ONLY_CIRCUIT_BETWEEN_THEM: tuple[str, ...] = ("west", "h", "east")
...
def test_a_circuit_no_site_and_no_city_would_miss_is_reported_with_the_miles_it_runs() -> None:
    assert removable_circuits(_published_network(_SQUARE + [_SHORT_CROSSING])) == [
        ("west -> g -> east", 200.0)
    ]
...
def test_a_circuit_the_operator_pinned_is_kept_though_the_three_demands_would_let_it_go() -> None:
    assert not removable_circuits(
        _published_network(_SQUARE + [_SHORT_CROSSING], (("west", "east"),))
    )


def test_a_pinned_pair_keeps_no_circuit_between_two_other_sites() -> None:
    assert removable_circuits(
...
def test_a_circuit_a_site_would_lose_a_diverse_circuit_by_is_kept() -> None:
    assert not removable_circuits(_published_network(_TRIANGLE))


def test_a_circuit_holding_the_two_halves_of_a_backbone_together_is_kept() -> None:
    assert not removable_circuits(
        _published_network(_TRIANGLE + _FAR_TRIANGLE + [_ONLY_CIRCUIT_BETWEEN_THEM])
    )

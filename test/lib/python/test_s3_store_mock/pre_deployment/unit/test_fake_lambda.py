...
def test_the_invocation_made_is_the_one_recorded() -> None:
    ...
    fake_lambda(invocations).invoke(FunctionName="wan-synthesizer-merge", Payload=b"{}")
    assert invocations == [{"FunctionName": "wan-synthesizer-merge", "Payload": b"{}"}]


def test_an_invocation_is_answered_as_an_accepted_asynchronous_call() -> None:
    assert fake_lambda([]).invoke(FunctionName="wan-synthesizer-merge") == {"StatusCode": 202}
